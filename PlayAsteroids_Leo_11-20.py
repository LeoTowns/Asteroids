...
class Ship(Shootable):
    ...
    # If you are not already invulnerable from recent death, pressing 's'
    # will set self.death_delay to some a value that decrements to 0. If shield
    # runs out of power, it will turn off and begin regenerating.
        MovingBody.update(self)
        targets = [a for a in self.world.agents if isinstance(a,Shootable)]
        for t in targets:
            if ( (t is not self) and t.is_hit_by(self) ):
                t.explode()
                break
                # Above code is adapted from update() method in Ship class.
        if self.recent_death == False:
            self.death_delay = 300
            # I wonder if 300 is too much? We'll see
            while self.death_delay > 0:
                self.death_delay -= 1
            while self.death_delay <= 300:
                self.death_delay += 1 
            # –– What is this shield method missing (aside from visual 
            # representation)?
            # –– Method should maybe involve update() in a different way?
            # –– Shield should also have a way to turn off using 's' again like 
...
print("Hit 'j' and 'l' to turn, 's' to deploy a shield, 'i' to create thrust, and SPACE to shoot.")
